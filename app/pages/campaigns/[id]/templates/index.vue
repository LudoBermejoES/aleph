<template>
  <div class="p-8 max-w-3xl">
    ...
    <div v-else-if="templateList.length" class="space-y-2">
      <div v-for="tpl in templateList" :key="tpl.id" class="flex items-center justify-between p-4 rounded-lg border border-border">
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-2 mt-1">
            <span class="text-xs bg-secondary text-secondary-foreground px-2 py-0.5 rounded">{{ tpl.entityTypeSlug }}</span>
            <span v-if="tpl.isDefault" class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded">{{ $t('templates.default') }}</span>
            <span class="text-xs text-muted-foreground">{{ tpl.fieldCount ?? 0 }} {{ $t('templates.fields') }}</span>
          </div>
        </div>
        <div v-if="canManage" class="flex gap-2 shrink-0">
          ...
          <Button variant="destructive" size="sm" @click="confirmDelete(tpl)">{{ $t('common.delete') }}</Button>
        </div>
      </div>
    </div>
    <EmptyState v-else icon="📋" :title="$t('templates.empty')" :description="$t('templates.emptyDescription')" />
