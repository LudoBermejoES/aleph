  ...
  <div class="p-8">
    ...
    <div v-else-if="shop">
      ...
      <div class="flex items-center justify-between mb-3">
        ...
        <button v-if="canEdit" @click="showAddForm = !showAddForm" class="px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-sm" data-testid="add-stock-btn">
          {{ showAddForm ? $t('common.cancel') : $t('shops.addStock') }}
        </button>
      </div>

      <!-- Add stock form -->
      <div v-if="showAddForm" class="mb-4 p-4 rounded-lg border border-border space-y-3" data-testid="add-stock-form">
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="text-sm font-medium block mb-1">{{ $t('transactions.item') }}</label>
            <select v-model="addForm.itemId" class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" data-testid="stock-item-select">
          ...
          <div>
            ...
            <div class="flex items-center gap-2">
              <input v-model.number="addForm.quantity" type="number" :min="addForm.quantity === -1 ? -1 : 0" class="flex-1 rounded-md border border-input bg-background px-3 py-2 text-sm" data-testid="stock-quantity" />
              <label class="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer">
                <input type="checkbox" :checked="addForm.quantity === -1" @change="addForm.quantity = ($event.target as HTMLInputElement).checked ? -1 : 1" data-testid="stock-unlimited" />
        ...
        <button @click="addStock" :disabled="!addForm.itemId || saving" class="px-4 py-2 rounded-md bg-primary text-primary-foreground text-sm disabled:opacity-50" data-testid="stock-add-save">
          {{ saving ? $t('common.saving') : $t('common.save') }}
        </button>
      </div>

      <div v-if="shop.stock?.length" class="space-y-2">
        <div v-for="item in shop.stock" :key="item.id" class="p-3 rounded border border-border" :data-testid="`stock-row-${item.id}`">
          <!-- View mode -->
          <div v-if="editingStockId !== item.id" class="flex items-center justify-between">
            <div>
              ...
              <span v-if="!item.isAvailable" class="text-xs ml-2 text-muted-foreground">({{ $t('shops.unavailable') }})</span>
            </div>
            <div class="flex items-center gap-3">
              <span class="text-sm text-muted-foreground">{{ item.quantity === -1 ? $t('shops.unlimited') : item.quantity }} {{ $t('shops.inStock') }}</span>
              <span v-if="item.priceOverrideJson || item.itemPriceJson" class="text-sm">{{ formatPrice(item.priceOverrideJson || item.itemPriceJson, currencyList) }}</span>
              <div v-if="canEdit" class="flex gap-2">
                <button @click="startEditStock(item)" class="text-sm text-muted-foreground hover:text-primary" :data-testid="`stock-edit-${item.id}`">{{ $t('common.edit') }}</button>
                <button @click="confirmRemoveStock(item)" class="text-sm text-muted-foreground hover:text-destructive" :data-testid="`stock-remove-${item.id}`">{{ $t('shops.removeStock') }}</button>
              </div>
            </div>
          </div>
          ...
          <div v-else class="space-y-3" :data-testid="`stock-edit-form-${item.id}`">
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-sm font-medium block mb-1">{{ $t('shops.quantity') }}</label>
                <div class="flex items-center gap-2">
                  <input v-model.number="editStockForm.quantity" type="number" class="flex-1 rounded-md border border-input bg-background px-2 py-1.5 text-sm" :data-testid="`stock-edit-qty-${item.id}`" />
                  <label class="flex items-center gap-1 text-xs cursor-pointer">
                    <input type="checkbox" :checked="editStockForm.quantity === -1" @change="editStockForm.quantity = ($event.target as HTMLInputElement).checked ? -1 : 1" />
                    {{ $t('shops.unlimitedLabel') }}
                  </label>
                </div>
              </div>
              <div>
                ...
                <label class="flex items-center gap-2 text-sm cursor-pointer mt-2">
                  <input type="checkbox" v-model="editStockForm.isAvailable" :data-testid="`stock-edit-avail-${item.id}`" />
                  {{ editStockForm.isAvailable ? $t('shops.available') : $t('shops.unavailable') }}
                </label>
              </div>
            </div>
            <div class="flex gap-2">
              <button @click="saveStock(item.id)" :disabled="saving" class="px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-sm disabled:opacity-50" :data-testid="`stock-edit-save-${item.id}`">
                {{ saving ? $t('common.saving') : $t('common.save') }}
              </button>
              <button @click="editingStockId = null" class="px-3 py-1.5 rounded-md border border-border text-sm" :data-testid="`stock-edit-cancel-${item.id}`">{{ $t('common.cancel') }}</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <Dialog v-model:open="showRemoveDialog">
      <DialogContent class="max-w-sm">
        ...
        <DialogFooter>
          <button @click="showRemoveDialog = false" class="px-3 py-1.5 rounded-md border border-border text-sm" data-testid="stock-remove-cancel">{{ $t('common.cancel') }}</button>
          <button @click="removeStock" :disabled="saving" class="px-3 py-1.5 rounded-md bg-destructive text-destructive-foreground text-sm disabled:opacity-50" data-testid="stock-remove-confirm">
...
function startEditStock(item: any) {
  ...
  editStockForm.value = { quantity: item.quantity, isAvailable: item.isAvailable ?? true, priceOverrideJson: item.priceOverrideJson || '' }
}
