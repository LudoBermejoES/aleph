    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        ...
        <NuxtLink :to="`/campaigns/${campaignId}/timelines/new`">
          <Button variant="outline" data-testid="new-timeline-btn">{{
            $t('calendars.newTimeline')
          }}</Button>
        </NuxtLink>
      </div>
    </div>
    ...
    <template v-else>
      <div v-if="calendarList.length" class="space-y-4 mb-8">
        ...
        <NuxtLink
          ...
          :key="cal.id"
          :to="`/campaigns/${campaignId}/calendars/${cal.id}`"
              ...
              <span class="text-sm text-muted-foreground ml-2">
                {{
                  $t('calendars.currentDate', {
                    year: cal.currentDate?.year || cal.currentYear,
                    month: cal.currentDate?.month || cal.currentMonth,
                    day: cal.currentDate?.day || cal.currentDay,
                  })
                }}
              </span>
            </div>
            <span class="text-xs text-muted-foreground">{{
              $t('calendars.monthsInfo', {
                months: cal.config?.months?.length || 0,
                moons: cal.moons?.length || 0,
              })
            }}</span>
          </div>
        </NuxtLink>
      ...
      <div v-if="timelineList.length" class="space-y-4">
        ...
        <NuxtLink
          v-for="tl in timelineList"
          :key="tl.id"
          :to="`/campaigns/${campaignId}/timelines/${tl.slug}`"
          class="block p-4 rounded-lg border border-border hover:border-primary/50 transition-colors"
        >
          <span class="font-medium">{{ tl.name }}</span>
          <span class="text-sm text-muted-foreground ml-2"
            >{{ tl.events?.length || 0 }} {{ $t('calendars.events') }}</span
          >
        </NuxtLink>
      </div>
      <EmptyState
        v-if="!calendarList.length && !timelineList.length"
        icon="📅"
        :title="$t('calendars.empty')"
