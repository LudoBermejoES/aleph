<template>
  <div class="p-8">
    ...
    >
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            v-model="form.name"
            :placeholder="$t('inventories.namePlaceholder')"
        ...
        <div>
          ...
          <select
            ...
            class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
            data-testid="inv-owner-type"
            :aria-label="$t('aria.filters.inventoryCreateOwnerType')"
          >
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            ...
            <span class="text-xs ml-2 px-2 py-0.5 rounded bg-secondary text-secondary-foreground">{{
              inv.ownerType
            }}</span>
          </div>
          <span class="text-sm text-muted-foreground"
            >{{ inv.items?.length ?? 0 }} {{ $t('inventories.items') }}</span
          >
        </div>
      </NuxtLink>
    </div>
    <EmptyState
      ...
      :description="$t('inventories.emptyDescription')"
    />
