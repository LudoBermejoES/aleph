    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <input
          ...
          @input="searchEntities('source')"
        />
        <div
          v-if="search.sourceResults.length"
          class="border border-border rounded mt-1 max-h-40 overflow-y-auto"
        >
          <button
        ...
        <p v-if="form.sourceEntityName" class="text-xs text-primary mt-1">
          Selected: {{ form.sourceEntityName }}
        </p>
      </div>
      <div>
        ...
        <input
          ...
          :placeholder="$t('relations.searchEntities')"
          class="w-full mt-1 px-3 py-2 rounded border border-input bg-background"
          @input="searchEntities('target')"
        />
        <div
          v-if="search.targetResults.length"
          class="border border-border rounded mt-1 max-h-40 overflow-y-auto"
        >
          <button
        ...
        <p v-if="form.targetEntityName" class="text-xs text-primary mt-1">
          Selected: {{ form.targetEntityName }}
        </p>
      </div>
    </div>
    <div>
