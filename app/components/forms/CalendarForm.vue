  ...
  <form @submit.prevent="$emit('submit')" class="space-y-6">
    ...
    <div class="grid grid-cols-3 gap-4">
      ...
      <div>
        ...
        <input
          v-model.number="form.currentMonth"
          type="number"
          min="1"
          class="w-full mt-1 px-3 py-2 rounded border border-input bg-background"
        />
    ...
    <div>
      <div class="flex items-center justify-between mb-2">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          @click="form.months.push({ name: '', days: 30 })"
          >{{ $t('calendars.addMonth') }}</Button
        >
      </div>
      <div v-for="(m, i) in form.months" :key="i" class="flex gap-2 mb-2">
        <input
          v-model="m.name"
          :placeholder="$t('calendars.monthNamePlaceholder')"
        ...
        <input
          ...
          type="number"
          min="1"
          :placeholder="$t('calendars.daysLabel')"
          class="w-24 px-3 py-2 rounded border border-input bg-background"
        />
        <Button
          type="button"
          variant="ghost"
          size="sm"
          @click="form.months.splice(i, 1)"
          class="text-red-500"
          >Remove</Button
        >
      </div>
      <p v-if="!form.months.length" class="text-sm text-muted-foreground">
        {{ $t('calendars.noMonths') }}
      </p>
    </div>
    <div>
      <label class="text-sm font-medium">{{ $t('calendars.weekdays') }}</label>
      <input
        v-model="form.weekdaysRaw"
        class="w-full mt-1 px-3 py-2 rounded border border-input bg-background"
        :placeholder="$t('calendars.weekdaysPlaceholder')"
      />
