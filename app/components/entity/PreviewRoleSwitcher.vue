<template>
  ...
  <div v-if="isDm">
    <!-- Preview mode banner -->
    <div v-if="previewRole" class="flex items-center justify-between gap-3 px-4 py-2 mb-4 rounded-lg bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700 text-sm">
      <span class="text-amber-800 dark:text-amber-200 font-medium">{{ $t('secrets.previewBanner', { role: previewRole }) }}</span>
      <button
        type="button"
        class="text-amber-700 dark:text-amber-300 underline text-xs hover:no-underline"
        @click="clearPreview"
      >{{ $t('secrets.previewAs') }}: DM</button>
    </div>
