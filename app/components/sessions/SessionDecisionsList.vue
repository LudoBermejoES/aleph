<template>
  <div class="mb-6">
    <div class="flex items-center justify-between mb-3">
      ...
      <Button v-if="canManage" variant="outline" size="sm" @click="showAddDecision = !showAddDecision">
    ...
    <div v-if="showAddDecision" class="mb-4 p-4 rounded-lg border border-border bg-muted/30">
      ...
      <div class="flex gap-2">
        <Button size="sm" :disabled="!newDecision.title.trim()" @click="submitDecision">{{ $t('common.save') }}</Button>
        ...
        <div class="p-3 rounded border border-border">
          ...
          <div v-if="(d as any).consequences?.length" class="mb-2 space-y-1">
            <div v-for="c in (d as any).consequences" :key="c.id" class="flex items-start gap-2 text-xs pl-3 border-l border-border">
              <span :class="(c.revealed || canManage) ? 'text-foreground' : 'text-muted-foreground italic'">
                {{ c.revealed ? c.description : canManage ? `[${$t('sessions.hiddenLabel')}] ${c.description}` : $t('sessions.hiddenConsequence') }}
              </span>
              <button v-if="canManage" :class="['ml-auto flex-shrink-0 text-xs px-1.5 py-0.5 rounded border transition-colors', c.revealed ? 'border-green-500 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20' : 'border-border text-muted-foreground hover:border-primary/50']"
          ...
          <div v-if="canManage">
            <button v-if="addingConsequenceFor !== d.id" class="text-xs text-muted-foreground hover:text-foreground transition-colors" @click="addingConsequenceFor = d.id">
              + {{ $t('sessions.addConsequence') }}
            </button>
            <div v-else class="mt-2 p-2 rounded border border-border bg-muted/30">
              <input v-model="newConsequence.description" type="text" class="w-full px-2 py-1 text-xs rounded border border-input bg-background mb-2" :placeholder="$t('sessions.consequencePlaceholder')" />
              <div class="flex items-center gap-3">
                <label class="flex items-center gap-1 text-xs">
                  <input v-model="newConsequence.revealed" type="checkbox" class="rounded" />
                  {{ $t('sessions.revealedByDefault') }}
                </label>
                <Button size="sm" :disabled="!newConsequence.description.trim()" @click="submitConsequence(d.id)">{{ $t('common.save') }}</Button>
...
function submitDecision() {
  emit('add-decision', { title: newDecision.value.title, type: newDecision.value.type, description: newDecision.value.description || undefined })
  newDecision.value = { title: '', type: 'choice', description: '' }
  showAddDecision.value = false
}

function submitConsequence(decisionId: string) {
  emit('add-consequence', decisionId, { description: newConsequence.value.description, revealed: newConsequence.value.revealed })
