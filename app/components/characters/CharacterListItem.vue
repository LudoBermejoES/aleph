    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <span
            class="inline-flex items-center gap-1 text-xs ml-2 px-2 py-0.5 rounded bg-secondary text-secondary-foreground"
          >
            <component
          ...
          <span v-if="character.class" class="text-xs ml-1 text-muted-foreground">{{
            character.class
          }}</span>
          <span v-if="character.alignment" class="text-xs ml-1 text-muted-foreground italic">{{
            character.alignment
          }}</span>
          <span v-if="character.isCompanionOf" class="text-xs ml-1 text-muted-foreground italic">{{
            $t('characters.companion')
          }}</span>
          <span
            v-if="character.locationName"
            class="inline-flex items-center gap-0.5 text-xs ml-2 text-muted-foreground"
            data-testid="location-indicator"
          >
            <component :is="ICONS.locations" class="w-3 h-3" />{{ character.locationName }}
          </span>
          <span
            v-if="character.primaryOrg"
            class="inline-flex items-center gap-1 text-xs ml-2 px-2 py-0.5 rounded bg-secondary text-secondary-foreground"
