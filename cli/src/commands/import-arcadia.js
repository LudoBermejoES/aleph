...
function apiGet(config, endpoint) {
  return apiFetch(config, 'GET', endpoint)
}
function apiPost(config, endpoint, body) {
  return apiFetch(config, 'POST', endpoint, body)
}
function apiPut(config, endpoint, body) {
  return apiFetch(config, 'PUT', endpoint, body)
}
...
// ---------------------------------------------------------------------------
async function importSessionContent(
  config,
  campaignId,
  sessionSlug,
  manualNotes,
  ...
  for (const [type, content] of uploads) {
    try {
      await apiPut(config, `/campaigns/${campaignId}/sessions/${sessionSlug}/content`, {
        type,
        content,
      })
    } catch (err) {
...
async function importCrematorio(config, campaignId, groupSlug) {
  ...
  const aiNotesFile = path.join(
    ARCADIA_ROOT,
    'crematorio-la-tranquilidad',
    'ai-notes',
    '2025-10-25-Session-1.md',
  )
