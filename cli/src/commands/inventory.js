...
export function makeInventoryCommand() {
  ...
  cmd
    ...
    .action(async (opts) => {
      ...
      print(opts.json ? data : data.map(i => ({ id: i.id, ownerType: i.ownerType, ownerId: i.ownerId || '' })), { json: opts.json })
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      const data = await post(`/api/campaigns/${opts.campaign}/inventories`, {
        ownerType: opts.ownerType, ownerId: opts.ownerId,
      })
      if (opts.json) { print(data, { json: true }) } else { success(`Inventory created: ${data.id}`) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      if (!opts.yes) {
        const ok = await confirm({ message: `Delete inventory ${opts.id}? This cannot be undone.`, default: false })
  ...
  cmd
    ...
    .option('--json', 'Output as JSON')
    .action(async (opts) => {
      const data = await post(`/api/campaigns/${opts.campaign}/inventories/${opts.inventory}/items`, {
        itemId: opts.item, quantity: opts.quantity,
      })
      if (opts.json) { print(data, { json: true }) } else { success('Item added to inventory.') }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      if (!opts.yes) {
        const ok = await confirm({ message: `Remove item ${opts.item} from inventory ${opts.inventory}?`, default: false })
        if (!ok) { process.stdout.write('Cancelled.\n'); return }
      }
  ...
  cmd
    ...
    .action(async (opts) => {
      const data = await post(`/api/campaigns/${opts.campaign}/inventories/${opts.from}/transfer`, {
        toInventoryId: opts.to, itemId: opts.item, quantity: opts.quantity,
      })
      if (opts.json) { print(data, { json: true }) } else { success('Transfer complete.') }
    })
