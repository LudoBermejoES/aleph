...
- **[Risk] The content-population pass is manual judgment, not mechanical** — deciding "does this session/arc actually mention this character/location" requires reading prose, unlike the code changes which are mechanical and testable. → **Mitigation**: scoped to Berlín en Tinieblas only (see Non-Goals), and the plan (Migration Plan below) is to work from the richest, most legible sources first (session summaries and arc descriptions already written in this repo) rather than re-deriving everything from raw transcripts.
