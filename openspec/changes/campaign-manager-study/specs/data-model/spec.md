# Aleph -- Data Model Specification

## Purpose

Define the SQLite database schema for all metadata, relationships, permissions, and indexes. Content lives in `.md` files on the filesystem; the database stores everything else. This spec covers all tables, their relationships, and the FTS5 search index.
...
```
users ─┬─< sessions
       ├─< campaign_members ─┬─< campaign_member_permissions
       │                     └──> campaigns ─┬─< entities ─┬─< entity_fields
       │                                     │             ├─< entity_tags ──> tags
       │                                     │             ├─< entity_names
       │                                     │             ├─< entity_relations
       │                                     │             ├─< entity_permissions
       │                                     │             └─< entity_specific_viewers
       │                                     ├─< maps ─┬─< map_layers
       │                                     │         ├─< map_pins
       │                                     │         └─< map_groups
       │                                     ├─< sessions_tbl ─┬─< session_attendance
       │                                     │                 └─< decisions
       │                                     ├─< arcs ─< chapters
       │                                     ├─< quests
