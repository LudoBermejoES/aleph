...
- [ ] 1.1 Add `shortDescription: text('short_description')` to `quests` in
      `server/db/schema/sessions.ts`, next to `description`.
- [ ] 1.2 Create `server/db/migrations/0038_quest_short_description.sql` with
...
- [ ] 1.3 Boot the server against an empty database and confirm the migration applies on its own.
...
- [ ] 2.1 Create `shared/utils/quest-short-description.ts` exporting
...
- [ ] 2.2 Unit test in `tests/unit/shared/` pinning the value, so it is the single place to change.
- [ ] 2.3 **Guard**: a test that walks the short-description code paths (zod schema, form) and fails
      if a literal `200` appears instead of the imported constant. This is what stops D5 eroding on
      the first hurried edit.

## 3. API

- [ ] 3.1 Add `shortDescription: z.string().max(QUEST_SHORT_DESCRIPTION_MAX_LENGTH).nullable().optional()`
      to the schema in `quests/index.post.ts`.
- [ ] 3.2 Same in `quests/[slug]/index.put.ts`, propagating with the `!== undefined` pattern the
...
- [ ] 3.3 Confirm both GETs (list and detail) return the field. If they project columns explicitly,
      add it; if they `select()`, record that this was verified rather than assumed.
- [ ] 3.4 Integration: create with a short description and **read it back**, comparing the exact
...
- [ ] 3.5 Integration: 201 characters → validation error, and assert **nothing was created or
      modified**.
- [ ] 3.6 Integration: sending `null` clears it; **omitting the key leaves it untouched**. These are
      two different cases and the second is the one that silently breaks.

## 4. UI — form and detail page

- [ ] 4.1 `app/types/api.ts`: `shortDescription: string | null` on `Quest`.
- [ ] 4.2 `app/components/forms/QuestForm.vue`: a single-line text field above the long description,
...
- [ ] 4.3 `app/pages/campaigns/[id]/quests/[slug]/index.vue`: standfirst above the description,
      typographically distinct, interpolated as PLAIN TEXT — never through `<MDC>` (design D2).
- [ ] 4.4 i18n in `i18n/locales/es.json` and `en.json`: label, placeholder and help text saying what
...
- [ ] 4.5 Check the detail page renders no empty gap when there is a standfirst and no long
      description, and none the other way round.

## 5. UI — the list

- [ ] 5.1 `app/pages/campaigns/[id]/quests/index.vue`: implement D4's fallback rule in this order —
      short description whole and **without `line-clamp`**; else an excerpt of the long one **with
      `line-clamp-2`**; else render nothing.
- [ ] 5.2 The two branches carry different CSS classes. Write that down in a comment: putting the
      clamp on both "just in case" trims the short description at narrow widths and breaks exactly
      what the field promises.
- [ ] 5.3 Extend the guard in `tests/unit/components/quest-list.test.ts` — the one that READS the
...
- [ ] 5.4 Mutation-test that guard: swap the classes and require red. Then **remove the mutation and
      `grep` to confirm no trace is left** — this project has already shipped a
      `data-testid="…-DISABLED"` that was an unreverted mutation.
- [ ] 5.5 Check that a 200-character short description with no spaces neither overflows the card nor
      causes horizontal page scroll at phone width.

## 6. CLI

- [ ] 6.1 `cli/src/commands/quest.js`: `--short-description <text>` on `create` and `update`,
      following the `--board-summary` pattern in `entity.js` (empty string → `null`).
- [ ] 6.2 Show the short description in `quest list`.
- [ ] 6.3 CLI integration: set the short description, **read it back** and compare; then clear it
      with `""` and confirm the long description is untouched.
- [ ] 6.4 Confirm that text longer than 200 characters makes the CLI exit non-zero **and print the
...
- [ ] 6.5 Update `docs/claude-skill.md` and `.claude/skills/aleph-cli/SKILL.md` together, bumping the
      latter's `version`. Run the endpoint-parity check if one exists.

## 7. Export / import — verified, not assumed

- [ ] 7.1 Export a campaign holding quests with and without a short description, re-import it, and
      **compare the field on both sides**. Reading `campaign-export.ts` and concluding that
      `select()` carries it does not count.
- [ ] 7.2 A unit or integration test pinning that round trip, so the field cannot vanish silently if
      someone later moves the export to explicit columns.

## 8. Closing out

- [ ] 8.1 E2E in `tests/e2e/quests.spec.ts`: create with a short description, see it whole in the
      list, plus a control confirming a quest without one still shows the excerpt.
- [ ] 8.2 State in the e2e file itself that **CI does not run Playwright** (it runs format, eslint,
      `tests/unit/`, integration and build), so nobody treats it as enforced.
- [ ] 8.3 `npm run format:check`, `npx eslint .` and the full unit suite — **on Node 25**:
...
- [ ] 8.4 Integration suite with the server running.
