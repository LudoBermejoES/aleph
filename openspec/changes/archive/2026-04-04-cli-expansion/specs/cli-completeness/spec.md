...
### Requirement: TypeScript CLI Codebase

All CLI source files SHALL be written in TypeScript, compiled to JavaScript for distribution.

#### Scenario: CLI builds without errors

- GIVEN the CLI source is in TypeScript
- WHEN `npm run build` is executed in the `cli/` directory
- THEN `tsc` compiles all files to `cli/dist/` without errors

#### Scenario: CLI runs from compiled output

- GIVEN the CLI has been built
- WHEN the user runs `node cli/bin/aleph.js --help`
- THEN the CLI displays the help message listing all commands

### Requirement: Standardized Interactive Prompts

All CLI commands that require interactive input SHALL use `@inquirer/prompts` exclusively.

#### Scenario: Confirmation prompt uses @inquirer/prompts

- GIVEN the user runs a destructive command (e.g., `aleph campaign delete`)
- WHEN the CLI prompts for confirmation
- THEN the prompt is rendered by `@inquirer/prompts` `confirm()` function
- AND handles Ctrl+C gracefully

#### Scenario: No readline or raw stdin usage

- GIVEN any CLI command file
- WHEN inspected for import statements
- THEN it does not import `readline` or read from `process.stdin` directly
