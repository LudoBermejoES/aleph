...
- [x] Create `scripts/swade-arcadia/` directory with `lib/` subdirectory
- [x] Create `scripts/swade-arcadia/lib/api.js` — reads `~/.aleph/config.json`, exports `apiFetch(path, method, body)` that calls `aleph.ludobermejo.es` with `X-API-Key` header; prints Spanish error and exits if config missing
- [x] Create `scripts/swade-arcadia/lib/templates.js` — exports all entity type definitions and template field arrays (see section 2 and 3)
- [x] Create `scripts/swade-arcadia/lib/format.js` — exports content formatters: `formatVentaja`, `formatDesventaja`, `formatRasgo`, `formatSuperpoder`, `formatArmadura`, `formatArma`, `formatEquipo`, `formatEscudo`, `formatVehiculo`, `formatBase`

## 2. Setup script — entity types and templates

- [x] Create `scripts/swade-arcadia/setup.js` with campaign ID hardcoded as `ARCADIA_CAMPAIGN_ID = '753b7958-d63b-4053-bcb5-1ac44b0f96e0'`
- [x] Implement `createEntityTypes()`: POST each of the 11 types to `/api/campaigns/:id/entity-types`; skip (log "ya existe, omitiendo") if slug already present
...
- [x] Implement `createEntityTemplates()`: POST a template for each entity type via `/api/campaigns/:id/entity-templates` with fields array; skip if name already present
...
- [x] Implement `createCharacterTemplates()`: POST two character templates to `/api/campaigns/:id/character-templates`
...
- [x] Print final summary: "✓ Configuración completada. X tipos de entidad, Y plantillas creadas."

## 3. Content formatters (lib/format.js)

- [x] `formatVentaja(v)` → markdown: `**Requisitos:** ${v.requisitos}\n\n${v.descripción}`
- [x] `formatDesventaja(d)` → markdown: `**Tipo:** ${d.tipo}\n\n${d.descripción}`
- [x] `formatRasgo(r)` → markdown: `**Atributo:** ${r.atributo}\n\n${r.descripción}`
- [x] `formatSuperpoder(sp, mdContent)` → if mdContent provided use it as base; prepend `**Coste:** ${sp.coste}\n\n**Ornamentos:** ${sp.ornamentos}\n\n`; append `## Modificadores\n` table from `sp.modificadores[]`; if no JSON match, use mdContent as-is
- [x] `formatArmadura(item, category)` → `**Protección:** ${item.armor} | **Localizaciones:** ${item.locations} | **Peso:** ${item.weight} kg | **Coste:** ${item.cost} mo\n\nCategoría: ${category}${item.notes ? '\n\n' + item.notes : ''}`
- [x] `formatArma(item, category)` → `**Daño:** ${item.damage} | **Fuerza Min.:** ${item.min_strength} | **Peso:** ${item.weight} kg | **Coste:** ${item.cost} mo\n\n${item.notes || ''}`
- [x] `formatEquipo(item, category)` → `**Peso:** ${item.weight ?? '-'} kg | **Coste:** ${item.cost} mo\n\nCategoría: ${category}${item.notes ? '\n\n' + item.notes : ''}`
- [x] `formatEscudo(item, category)` → `**Bonus Parada:** ${item.parry_bonus ?? '-'} | **Cobertura:** ${item.coverage ?? '-'} | **Peso:** ${item.weight} kg | **Coste:** ${item.cost} mo`
- [x] `formatVehiculo(item, category)` → `**Tamaño:** ${item.size} | **Manejo:** ${item.handling} | **Vel. Máx.:** ${item.topspeed} km/h | **Dureza:** ${item.toughness} | **Tripulación:** ${item.crew}\n\nCategoría: ${category}`
- [x] `formatBase(base)` → content describing base with cost per level and list of available modifications

## 4. Seed script — ventajas and desventajas

- [x] Create `scripts/swade-arcadia/seed-entities.js` with `--type` and `--all` CLI argument parsing (use `process.argv`)
- [x] Implement `seedVentajas()`:
...
- [x] Implement `--all` flag: runs all seed functions in sequence with progress output
- [x] Implement `--type <nombre>` single-type seeding: maps type names (ventajas, desventajas, rasgos, superpoderes, armaduras, armas, escudos, equipo, vehiculos, bases) to their seed functions
- [x] Add top-level error handler: print error message in Spanish and exit with code 1 on API failures
- [x] Add small delay (100ms) between entity creates to avoid rate-limiting the server

## 8. README

- [x] Create `scripts/swade-arcadia/README.md` with sections:
