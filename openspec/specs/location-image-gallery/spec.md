# location-image-gallery Specification

## Purpose

Many images per location instead of one: an `entity_images` table with an exactly-one-primary
invariant enforced by a partial unique index, list/upload/serve/update/delete endpoints under
`/api/campaigns/:id/locations/:slug/images`, captions and editor-controlled ordering, the primary
image mirrored into `entities.imageUrl` so every existing consumer shows it unchanged, a gallery
panel on the location detail and edit pages, a migration that turns each location's existing
single image into its primary, and aleph-cli commands.
