...
Eliminate N+1 query patterns in list endpoints and the genealogy service by replacing per-row child queries with batched `inArray`-based queries, and add missing foreign-key indexes to tables queried by `campaignId`.
