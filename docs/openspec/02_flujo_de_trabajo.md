...
AI:  Created openspec/changes/add-quest-objectives/
     ✓ proposal.md
     ✓ specs/quests/spec.md
     ✓ design.md
     ✓ tasks.md
...
AI:  Implementing tasks...
     ✓ 1.1 Add objectives field to quests DB schema
     ✓ 1.2 Add API endpoint PATCH /quests/:slug/objectives
     ✓ 2.1 Add QuestObjectives component
     All tasks complete!
...
Tú: Cómo deberíamos implementar notificaciones en tiempo real para sesiones.

AI:  Déjame revisar el código actual...
     [Analiza server/api/campaigns/[id]/sessions/, app/pages/.../sessions/]

     Aleph ya tiene Hocuspocus para colaboración en tiempo real (documentos).
     Para notificaciones de sesión hay dos opciones:
     - Extender Hocuspocus con awareness events (consistente con lo existente)
     - SSE endpoint dedicado (más simple para notificaciones unidireccionales)
...
```
Ejemplo:
- "Añadir filtros a la lista de personajes" → añadir un campo extra = actualizar
- "Añadir filtros a la lista de personajes" → decidir reescribir el sistema de búsqueda = NUEVO change
```
