...
export const campaignMembers = sqliteTable(
  ...
  (table) => [uniqueIndex('campaign_members_unique').on(table.campaignId, table.userId)],
)

export const campaignMemberPermissions = sqliteTable(
  'campaign_member_permissions',
  {
    id: text('id').primaryKey(),
  ...
  (table) => [uniqueIndex('member_permission_unique').on(table.campaignMemberId, table.permission)],
)
