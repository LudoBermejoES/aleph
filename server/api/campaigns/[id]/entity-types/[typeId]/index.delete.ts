...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'co_dm'))
    throw createError({ statusCode: 403, message: 'Co-DM or above can delete entity types' })
  ...
  if (entityType.isBuiltin)
    throw createError({ statusCode: 400, message: 'Cannot delete a builtin entity type' })
  ...
  if (used.length > 0)
    throw createError({
      statusCode: 409,
      message: `Entity type is in use by ${used.length} entities`,
    })
