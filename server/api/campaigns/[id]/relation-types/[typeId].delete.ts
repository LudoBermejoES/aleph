...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'dm'))
    throw createError({ statusCode: 403, message: 'Only DM can delete relation types' })
  ...
  if (type.isBuiltin)
    throw createError({ statusCode: 403, message: 'Cannot delete built-in relation types' })
