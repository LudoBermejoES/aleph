...
export default defineEventHandler(async (event) => {
  ...
  const origStats = db
    .select()
    .from(characterStats)
    .where(eq(characterStats.characterId, origChar.id))
    .all()
  const origAbilities = db
    .select()
    .from(abilities)
    .where(eq(abilities.characterId, origChar.id))
    .all()
  ...
  let origFile
  try {
    origFile = await readEntityFile(origEntity.filePath)
  } catch {
    origFile = { frontmatter: {}, content: '' }
  }
  ...
  db.insert(entities)
    .values({
      ...
      visibility: origEntity.visibility,
      contentHash: hash,
  ...
  db.insert(characters)
    .values({
      ...
      characterType: origChar.characterType,
      race: origChar.race,
      class: origChar.class,
      alignment: origChar.alignment,
      status: origChar.status,
      ownerUserId: null,
      isCompanionOf: origChar.isCompanionOf,
    })
    .run()
  ...
  for (const ab of origAbilities) {
    db.insert(abilities)
      .values({
        ...
        tagsJson: ab.tagsJson,
        sortOrder: ab.sortOrder,
        isSecret: ab.isSecret,
      })
      .run()
  }
