...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'player'))
    throw createError({ statusCode: 403, message: 'Players or above can withdraw from shop till' })
  ...
  if (!shop.isPlayerOwned)
    throw createError({ statusCode: 400, message: 'This shop is not player-owned' })
  ...
  if (role === 'player' && shop.ownedByUserId !== userId) {
    throw createError({
      statusCode: 403,
      message: "You can only withdraw from your own shop's till",
    })
  }
  ...
  const till = db
    ...
    .where(
      and(
        eq(wealth.ownerId, shop.id),
        eq(wealth.ownerType, 'shop'),
  ...
  // Deduct from till
  db.update(wealth)
    .set({ amount: till.amount - amount })
    .where(eq(wealth.id, till.id))
    .run()
  ...
  if (toOwnerId && toOwnerType) {
    const dest = db
      ...
      .where(
        and(
          eq(wealth.ownerId, toOwnerId),
          eq(wealth.ownerType, toOwnerType),
          eq(wealth.currencyId, currencyId),
        ),
      )
      .get()
    if (dest) {
      db.update(wealth)
        .set({ amount: dest.amount + amount })
        .where(eq(wealth.id, dest.id))
        .run()
    } else {
      db.insert(wealth)
        .values({
