...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'player'))
    throw createError({ statusCode: 403, message: 'Players or above can buy items' })

  const campaignId = getRouterParam(event, 'id')!
  const slug = getRouterParam(event, 'slug')!
  const buySchema = z.object({
    stockId: z.string(),
    buyerInventoryId: z.string(),
    buyerOwnerId: z.string(),
    buyerOwnerType: z.string(),
    quantity: z.number().optional(),
    currencyId: z.string(),
    price: z.number().optional(),
  })
  const body = await validateBody(event, buySchema)
  ...
  if (!stock || !stock.isAvailable)
    throw createError({ statusCode: 404, message: 'Stock item not available' })
  ...
  const buyerWealth = db
    .select()
    .from(wealth)
    .where(
      and(
        eq(wealth.ownerId, buyerOwnerId),
        eq(wealth.ownerType, buyerOwnerType),
  ...
  if (buyerWealth) {
    db.update(wealth)
      .set({ amount: buyerWealth.amount - price })
      .where(eq(wealth.id, buyerWealth.id))
      .run()
  }
  ...
  if (stock.quantity !== -1) {
    db.update(shopStock)
      .set({ quantity: stock.quantity - qty })
  ...
  const existing = db
    ...
    .where(
      and(
        eq(inventoryItems.inventoryId, buyerInventoryId),
        eq(inventoryItems.itemId, stock.itemId),
      ),
    )
    .get()
