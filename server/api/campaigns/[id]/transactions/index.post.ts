...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'editor'))
    throw createError({ statusCode: 403, message: 'Editors or above can log transactions' })
    ...
    for (const [currencyId, amount] of Object.entries(body.amounts as Record<string, number>)) {
      ...
      if (delta > 0 && body.toOwnerId && body.toOwnerType) {
        const existing = db
          ...
          .where(
            and(
              eq(wealth.ownerId, body.toOwnerId),
              eq(wealth.ownerType, body.toOwnerType),
        ...
        if (existing) {
          db.update(wealth)
            .set({ amount: existing.amount + delta })
            .where(eq(wealth.id, existing.id))
            .run()
        } else {
          db.insert(wealth)
            .values({
              id: randomUUID(),
              ownerType: body.toOwnerType,
              ownerId: body.toOwnerId,
              currencyId,
              amount: delta,
            })
            .run()
      ...
      if (delta < 0 && body.fromOwnerId && body.fromOwnerType) {
        const existing = db
          ...
          .where(
            and(
              eq(wealth.ownerId, body.fromOwnerId),
              eq(wealth.ownerType, body.fromOwnerType),
              eq(wealth.currencyId, currencyId),
            ),
          )
          .get()
