...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'dm'))
    throw createError({ statusCode: 403, message: 'Only DM can advance calendar date' })
  ...
  const currentDate: CalendarDate = cal.currentDateJson
    ? JSON.parse(cal.currentDateJson)
