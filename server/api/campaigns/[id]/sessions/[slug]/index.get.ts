...
import { gameSessions, sessionAttendance, sessionGroups, sessionContents } from '../../../../../db/schema/sessions'
...
export default defineEventHandler(async (event) => {
  ...
  if (session.groupId) {
    const group = db.select({ name: sessionGroups.name }).from(sessionGroups)
      .where(eq(sessionGroups.id, session.groupId)).get()
  ...
  // hasContent flags
  const contentRows = db.select({ type: sessionContents.type }).from(sessionContents)
    .where(eq(sessionContents.sessionId, session.id)).all()
  const hasContent = {
