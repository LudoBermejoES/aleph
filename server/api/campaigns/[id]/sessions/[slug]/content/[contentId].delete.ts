...
export default defineEventHandler(async (event) => {
  ...
  if (!hasMinRole(role, 'co_dm')) throw createError({ statusCode: 403, message: 'Co-DM or above can delete session content' })
  ...
  const content = db.select().from(sessionContents)
