...
describe('Search Permission Filtering (6.11)', () => {
  ...
  beforeAll(async () => {
    ...
    await api('/api/auth/sign-up/email', {
      method: 'POST',
      body: { name: 'MD DM', email: dmEmail, password: 'password123' },
    })
    ...
    await api('/api/auth/sign-up/email', {
      method: 'POST',
      body: { name: 'MD Player', email: playerEmail, password: 'password123' },
    })
  ...
  it('DM search returns all entities including dm_only', async () => {
    const res = await api(`/api/campaigns/${campaignId}/search?q=Entity`, {
      method: 'GET',
...
describe('Secret Block Rendering (6.12, 6.13)', () => {
  ...
  beforeAll(async () => {
    ...
    const camp = await api('/api/campaigns', {
      method: 'POST',
      headers: withCsrf(dmCookie, dmCsrf),
      body: { name: `Secret Test ${Date.now()}` },
    })
    campaignId = (await camp.json()).id
