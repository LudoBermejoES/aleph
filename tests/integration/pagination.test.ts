...
describe('Pagination integration tests', () => {
  ...
  beforeAll(async () => {
    await api('/api/auth/sign-up/email', { method: 'POST', body: { name: 'Pagination Tester', email, password: 'password123' } })
  ...
  describe('characters endpoint', () => {
    it('returns paginated response with meta when pageSize > 0', async () => {
      const res = await api(`/api/campaigns/${campaignId}/characters?pageSize=2`, { headers: { Cookie: cookie } })
    ...
    it('returns page 2 correctly', async () => {
      const res = await api(`/api/campaigns/${campaignId}/characters?pageSize=2&page=2`, { headers: { Cookie: cookie } })
      expect(res.status).toBe(200)
      const body = await res.json()
    ...
    it('pageSize=0 returns flat array (backward compat)', async () => {
      const res = await api(`/api/campaigns/${campaignId}/characters?pageSize=0`, { headers: { Cookie: cookie } })
      expect(res.status).toBe(200)
      const body = await res.json()
    ...
    it('page beyond total returns empty data with correct meta', async () => {
      const res = await api(`/api/campaigns/${campaignId}/characters?pageSize=50&page=999`, { headers: { Cookie: cookie } })
      expect(res.status).toBe(200)
      const body = await res.json()
  ...
  describe('organizations endpoint', () => {
    ...
    it('returns paginated response with meta', async () => {
      const res = await api(`/api/campaigns/${campaignId}/organizations?pageSize=10`, { headers: { Cookie: cookie } })
      expect(res.status).toBe(200)
      const body = await res.json()
      expect(Array.isArray(body.data)).toBe(true)
      expect(body.meta).toBeDefined()
    ...
    it('pageSize=0 returns flat array (backward compat)', async () => {
      const res = await api(`/api/campaigns/${campaignId}/organizations?pageSize=0`, { headers: { Cookie: cookie } })
      expect(res.status).toBe(200)
      const body = await res.json()
      expect(Array.isArray(body)).toBe(true)
    })
  ...
  describe('maps endpoint', () => {
    it('returns paginated response with meta', async () => {
      const res = await api(`/api/campaigns/${campaignId}/maps?pageSize=10`, { headers: { Cookie: cookie } })
      expect(res.status).toBe(200)
      const body = await res.json()
      expect(Array.isArray(body.data)).toBe(true)
      expect(body.meta).toBeDefined()
