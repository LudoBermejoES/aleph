...
describe('Entity RBAC (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    await api('/api/auth/sign-up/email', {
      method: 'POST',
      body: { name: 'RBAC DM', email: dmEmail, password: 'password123' },
    })
    ...
    const camp = await api('/api/campaigns', {
      ...
      body: { name: `RBAC Entity Test ${Date.now()}` },
    })
    ...
    await api('/api/auth/sign-up/email', {
      method: 'POST',
      body: { name: 'RBAC Player', email: playerEmail, password: 'password123' },
    })
...
describe('Template & Tag CRUD (integration)', () => {
  ...
  beforeAll(async () => {
    await api('/api/auth/sign-up/email', {
      method: 'POST',
      body: { name: 'Tmpl Tester', email, password: 'password123' },
    })
  ...
  it('decision and consequence recording', async () => {
    ...
    // Add consequence
    const conRes = await api(
      `/api/campaigns/${campaignId}/sessions/${sess.slug}/decisions/${dec.id}/consequences`,
      {
        method: 'POST',
        headers: withCsrf(cookie, csrfToken),
