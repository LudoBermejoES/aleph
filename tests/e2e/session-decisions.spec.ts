...
test.describe('Session Decisions', () => {
  test('DM can create a decision with a consequence and reveal it', async ({ page }) => {
    ...
    await expect(page.locator('main h1').first()).toContainText('Decision Test Session', {
      timeout: 10000,
    })
    ...
    await expect(page.locator('main').getByText('The party chose the forest path')).toBeVisible({
      timeout: 5000,
    })
    ...
    await expect(page.locator('main').getByText('They encounter a dragon later')).toBeVisible({
      timeout: 5000,
    })
  })
})
