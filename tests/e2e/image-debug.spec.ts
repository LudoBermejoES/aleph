...
const TINY_PNG = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI6QAAAABJRU5ErkJggg==', 'base64')

test('debug image upload', async ({ page }) => {
  const requests: string[] = []
  page.on('request', req => { if (req.url().includes('/api/')) requests.push(`${req.method()} ${req.url()}`) })
  page.on('response', res => { if (res.url().includes('/api/')) requests.push(`  → ${res.status()} ${res.url()}`) })
  page.on('console', msg => { requests.push(`CONSOLE ${msg.type()}: ${msg.text()}`) })
