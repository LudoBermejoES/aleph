...
test('detail page shows correct subtype, not "Other"', async ({ page }) => {
  ...
  await expect(page.locator('main').getByText('Region', { exact: false })).toBeVisible({
    timeout: 10000,
  })
...
test('list page shows correct subtype badge, not "Other"', async ({ page }) => {
  ...
  await expect(page.locator('main span.capitalize', { hasText: 'Country' }).first()).toBeVisible({
    timeout: 10000,
  })
  await expect(page.locator('main span.capitalize', { hasText: 'Dungeon' }).first()).toBeVisible({
    timeout: 10000,
  })
})
...
test('subtype is preserved after editing a location', async ({ page }) => {
  ...
  await expect(page.locator('main span.capitalize', { hasText: /^City$/ })).toBeVisible({
    timeout: 10000,
  })
